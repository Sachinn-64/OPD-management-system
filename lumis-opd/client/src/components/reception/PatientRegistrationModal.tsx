...
import { X, User, Phone, MapPin, Heart, UserPlus } from 'lucide-react';
...
export const PatientRegistrationModal: React.FC<PatientRegistrationModalProps> = ({ onClose, onSuccess, onError }) => {
  const [formData, setFormData] = useState<Omit<CreatePatientData, 'dateOfBirth'> & { age: string }>({
    firstName: '',
    middleName: '',
    lastName: '',
    age: '',
  ...
  const createPatientMutation = useMutation({
    mutationFn: (data: Omit<CreatePatientData, 'dateOfBirth'> & { age: string }) => {
      ...
      return patientService.create(transformedData as CreatePatientData);
    },
    onSuccess: (patient) => {
      onSuccess(`Patient registered successfully! UHID: ${patient.uhid}`);
    },
    onError: (err: any) => {
      onError(err.response?.data?.message || 'Failed to register patient');
    },
  });
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-2xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
        <div className="sticky top-0 bg-white border-b border-emerald-100 px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-gray-900">New Patient</h3>
              <p className="text-xs sm:text-sm text-gray-500">Register a new patient</p>
            </div>
          </div>
